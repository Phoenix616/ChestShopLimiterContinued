...
public class ItemStackBuilderCondition {
     ...
     */
    public ItemStackBuilder applyCondition(ItemStackBuilder itemStackBuilder, boolean condition, @Nullable Runnable onConditionApplied) {
        ...
        for (Map.Entry<ChangeType, Object> changesEntry : changeMap.entrySet()) {
            ...
            switch (changesEntry.getKey()) {
                case MATERIAL:
                    itemStackBuilder.setMaterial(Material.matchMaterial((String) value));
                    break;
                case NAME:
        ...
        if (onConditionApplied != null) onConditionApplied.run();
        return itemStackBuilder;
    }
