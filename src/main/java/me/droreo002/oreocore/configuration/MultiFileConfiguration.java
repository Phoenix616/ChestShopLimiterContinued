...
public class MultiFileConfiguration implements Configuration {
    ...
    public @NotNull FileConfiguration getConfig(@Nullable String filePath) {
        ...
        return this.loadedConfigs.computeIfAbsent(filePath, (s) -> {
            throw new NullPointerException("Cannot find config at " + filePath + ", please make sure the main folder name is not included.");
        });
    }
    ...
    public void setupConfig() {
        ...
        FileUtils.copyResourcesRecursively(super.getClass().getResource("/" + configFolder.getName()), configFolder);
        FileUtils.getListFile(configFolder, this.configFiles);
        reloadConfig();
    }
}
